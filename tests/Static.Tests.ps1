...
if ($ids.Count -ne 30 -or ($ids|Sort-Object -Unique).Count -ne $ids.Count) { throw 'Expected 26 active + 4 legacy unique tweak IDs.' }
...
Write-Host "PASS: v0.5 alpha app/modules/bootstrap parse clean, 26 active + 4 legacy tweaks, Profiles present." -ForegroundColor Green