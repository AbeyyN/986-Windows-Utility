...
scanner = root / 'storage' / 'scanner' / 'StorageScanner.cs'
test = root / 'tests' / 'StorageScanner.Tests.ps1'
...
marker = "Write-Host 'PASS: Storage scanner compiles and classifies synthetic files deterministically.' -ForegroundColor Green\n"
if marker not in ps:
    raise SystemExit('StorageScanner test marker missing')
guards = """$scannerSourceText = Get-Content $source -Raw -Encoding UTF8
...
"""
if guards not in ps:
    ps = ps.replace(marker, guards + marker, 1)
test.write_text(ps, encoding='utf-8', newline='\n')

cl = changelog.read_text(encoding='utf-8')
anchor = '- Fixed Win32 command-line quoting for fixed-drive roots such as `C:\\`; trailing backslashes are no longer placed inside naive quotes that can corrupt scanner arguments.\n'
addition = anchor + '- Reworked full-volume scanning onto one-pass Win32 `FindFirstFileExW` / `FindNextFileW` enumeration so file size comes directly from directory records instead of a separate managed `FileInfo` lookup per file.\n'
if cl.count(anchor) != 1:
    raise SystemExit('RC3 changelog scanner anchor missing or duplicated')
changelog.write_text(cl.replace(anchor, addition, 1), encoding='utf-8', newline='\n')

print('STORAGE_SCANNER_NATIVE_ENUM_PATCHED')
